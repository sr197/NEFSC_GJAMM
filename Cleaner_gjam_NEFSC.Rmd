...
specnames_6 <- as.data.frame(specnames_6)
specnames_6$sixmo <- 1
ycommon_all <- left_join(ycommon_all, specnames_6, by =c("ycommon_2"= "specnames_6"))


sum_m_spec <- ycommon_all[ycommon_all$ycommon_2 %in% specnames_6,] 
...
specnames_large <- as.vector(sum_tot_nozero$V1)
...
spec_coords_1 <- data.frame(name=rep(0,133), x=rep(0,133), y=rep(0,133))

for (i in 1:length(specnames_6)) { #for each species in spec_list
  wt.try <- wt.centroid(total_sp_1, specnames_6[i])
  name <- specnames_6[i]
...
spec_coords_2 <- data.frame(name=rep(0,133), x=rep(0,133), y=rep(0,133))
...
write.csv(dist_total, "dist_total_full_2.csv")
...
```{r}
#this is all of the data we would need only looking at species that are present in at least 6 months in both time periods! Got significance!!!
